...
use crate::pose_estimation::{SPINEPOSE_KEYPOINTS, read_spinepose_people};
use crate::train::PoseBatch;

type SpinePosePerson = Vec<[f32; 3]>;
type SpinePosePeople = Vec<SpinePosePerson>;
type SpinePosePeopleByImage = HashMap<u64, SpinePosePeople>;
...
impl CocoPoseDataset {
    ...
    fn from_coco_with_pose_source(
        ...
        let spineposes = spinepose_root
            .map(|root| load_spinepose_people_by_image(root, &images))
            .transpose()?;
...
fn load_spinepose_people_by_image(
    spinepose_root: &Path,
    images: &HashMap<u64, CocoImage>,
) -> Result<SpinePosePeopleByImage, PoseDataError> {
    let mut output = HashMap::new();
    for (image_id, image) in images {
        let path = spinepose_path_for_image(spinepose_root, &image.file_name);
        if !path.is_file() {
            continue;
        }
